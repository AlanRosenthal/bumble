...
from bumble.device import Advertisement, ConnectionParametersPreferences, Device, Connection, Peer
from bumble.utils import AsyncRunner
from bumble.transport import open_transport_or_link
from bumble.gatt import GATT_HEART_RATE_SERVICE, GATT_HEART_RATE_MEASUREMENT_CHARACTERISTIC, Characteristic, Service, CharacteristicDeclaration, Descriptor
from bumble.gatt_client import CharacteristicProxy
from bumble.smp import SMP_Pairing_Failed_Command, SMP_PAIRING_NOT_SUPPORTED_ERROR
...
class ConsoleApp:
    ...
    def __init__(self):
        ...
        self.treadmill_text = FormattedTextControl()
        ...
        container = HSplit(
            [
                ...
                ConditionalContainer(
                    Frame(Window(self.treadmill_text), title='Treadmill'),
                    filter=Condition(lambda: self.top_tab == 'treadmill'),
                ),
                ConditionalContainer(
        ...
        async with await open_transport_or_link(transport) as (hci_source, hci_sink):
            if device_config:
                ...
                self.device = Device.with_hci(
                    'Bumbleton', random_address, hci_source, hci_sink
                )
    ...
    async def do_show(self, params):
        ...
        while self.top_tab == 'treadmill':
            await self.do_show_treadmill()
            await asyncio.sleep(1)
    ...
    async def do_show_treadmill(self):
        prettytable = PrettyTable(
            field_names=[
                "Time",
                "Value",
        ...
        for index, item in enumerate(self.treadmill_values[-10:]):
            row = [f"{item['timestamp']}", item['value'], f"0x{hex(item['value'])}"]
            prettytable.add_row(row)
        
        self.treadmill_text.text = prettytable.get_string()
        self.ui.invalidate()
...
class DeviceListener(Device.Listener, Connection.Listener):
    ...
    async def on_connection(self, connection: Connection):
        ...
        self.app.append_to_output("Starting service discovery...")
        await self.app.connected_peer.discover_all()
        self.app.append_to_output("Service discovery complete!")

        # services = self.app.connected_peer.get_services_by_uuid(GATT_HEART_RATE_SERVICE)
        # if not services:
        #     self.app.show_error('Did not find the HR service')
        #     return

        # characteristic = self.app.connected_peer.get_characteristics_by_uuid(uuid=GATT_HEART_RATE_MEASUREMENT_CHARACTERISTIC, services=services)
        # if characteristic is None:
        #     self.app.show_error('Did not find the HR Measurement characteristic')
        #     return
        # hr_measurement_characteristic = characteristic[0]
        # self.app.append_to_output(f"HR measurement characteristic attribute: {hr_measurement_characteristic.handle}")

        hr_measurement_characteristics = self.app.connected_peer.get_characteristics_by_uuid(uuid=GATT_HEART_RATE_MEASUREMENT_CHARACTERISTIC, service=GATT_HEART_RATE_SERVICE)
        ...
        await hr_measurement_characteristic.subscribe(
            lambda value: 
                self.app.treadmill_values.append({
                    'value': int.from_bytes(value),
                    'timestamp': datetime.datetime.now()
                })
        )

        self.ui.create_background_task(app.command(["show", "treadmill"]))

    def on_security_request(self, _):
        # self.app.append_to_output("Rejecting security request!")
        # self.app.device.smp_manager.send_command(self.app.connected_peer.connection,
        #                                      SMP_Pairing_Failed_Command(reason=SMP_PAIRING_NOT_SUPPORTED_ERROR))
        self.app.append_to_output("Received security request!")
        asyncio.create_task(self.app.connected_peer.connection.pair())
    ...
    def on_advertisement(self, advertisement: Advertisement):
        uuids = advertisement.data.get(
            bumble.core.AdvertisingData.COMPLETE_LIST_OF_16_BIT_SERVICE_CLASS_UUIDS,
        )
        if not uuids:
            return
...
def main(device_config, transport):
    ...
    file_handler = logging.FileHandler('treadmill.log')
